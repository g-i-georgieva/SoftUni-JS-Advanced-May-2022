function solve() {
  const [checkBtn, clearBtn] = Array.from(
    document.querySelectorAll("#exercise>table>tfoot button")
  );
  ...
  function isValidColumn(arrOfRows) {
    for (let i = 0; i < arrOfRows[0].children.length; i++) {
      const curr = arrOfRows
        .map((e) => Array.from(e.children)[i])
  ...
  function checkHandler() {
    const isValidRow = Array.from(tbodyElem.children)
      .map(isValidLine)
      .every((e) => e == true);

    if (
      isValidRow == true &&
      isValidColumn(Array.from(tbodyElem.children)) == true
    ) {
