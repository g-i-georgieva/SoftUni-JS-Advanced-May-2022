function solve() {
  ...
  function addHandler(e) {
    ...
    if (
      ...
      const strongPrice = createStrong(
        `${Number(moviePriceElem.value).toFixed(2)}`
      );
      const inputPlaceholder = createInput("", [
  ...
  function archiveHandler(e) {
    ...
    if (inputElem.value != "" && isNaN(Number(inputElem.value)) == false) {
      ...
      const strongTotal = createStrong(
        `Total amount: ${(
          Number(priceElem.textContent) * Number(inputElem.value)
        ).toFixed(2)}`
      );
